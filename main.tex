...
\item Hodnost $\text{rank}(\mathbf{A}) < n$
\item Existuje nenulový vektor $\mathbf{v} \neq \theta$ takový, že $\mathbf{A}\mathbf{v} = \theta$
\end{itemize}
\end{itemize}
...
\item {\bf 1. krok:} Spočítáme informační zisk pro každý příznak (resp. sloupec) a vybereme, kde je informační zisk největší.
...
Entropie podmnožin:
\begin{itemize}
\item Pro $\mathcal{D}_1$: $p_1 = 0$, $p_0 = 1$ $\Rightarrow$ $H(\mathcal{D}_1) = -0 \cdot \log_2 0 - 1 \cdot \log_2 1 = 0$ bit 
...
kde $w_i, \ i \in \{0, 1,2,...p\}$ jsou nějaké neznámé koeficienty, které hledáme. $\varepsilon$ je náhodná veličina s $\mathbb{E}(\varepsilon) = 0$. Koeficient $w_0$ se nazývá {\bf intercept} a odpovídá (očekávané) výchozí hodnotě $Y$ při nulových příznacích.
...
&= \mathbf{Y}^T\mathbf{Y} - \underbrace{\mathbf{Y}^T\mathbf{X}\mathbf{w}}_\text{skalár} - \underbrace{\mathbf{w}^T\mathbf{X}^T\mathbf{Y}}_\text{skalár} + \mathbf{w}^T\mathbf{X}^T\mathbf{X}\mathbf{w} \\
&= \mathbf{Y}^T\mathbf{Y} - \underbrace{2\mathbf{Y}^T\mathbf{X}\mathbf{w}}_\text{skalár} + \mathbf{w}^T\mathbf{X}^T\mathbf{X}\mathbf{w}
\end{align*}
...
{\bf Pozn. k derivaci druhého členu:} Člen $-2\mathbf{Y}^T\mathbf{X}\mathbf{w}$ je skalár, proto platí $\mathbf{Y}^T\mathbf{X}\mathbf{w} = (\mathbf{Y}^T\mathbf{X}\mathbf{w})^T = \mathbf{w}^T\mathbf{X}^T\mathbf{Y}$. Derivace lineárního členu $\mathbf{w}^T\mathbf{c}$ podle $\mathbf{w}$ je $\mathbf{c}$, tedy $\frac{\partial}{\partial \mathbf{w}}(-2\mathbf{w}^T\mathbf{X}^T\mathbf{Y}) = -2\mathbf{X}^T\mathbf{Y}$.

{\bf Pozn. k derivace třetího členu:} Toto platí $\frac{\partial}{\partial \mathbf{w}}(\mathbf{w}^T\mathbf{X}^T\mathbf{X}\mathbf{w}) = 2\mathbf{X}^T\mathbf{X}\mathbf{w}$, neboť $\mathbf{X}^T\mathbf{X}$ je symetrická matice. Pro symetrickou matici $\mathbf{A}$ a libovolný vektor $\mathbf{w}$ obecně platí následující: 
...
kde $\mathbf{U} \in \mathbb{R}^{N \times N}$ a $\mathbf{V} \in \mathbb{R}^{(p+1) \times (p+1)}$ jsou ortogonální matice ($\mathbf{U}^T\mathbf{U} = \mathbf{I}$, $\mathbf{V}^T\mathbf{V} = \mathbf{I}$) a $\boldsymbol{\Sigma} \in \mathbb{R}^{N \times (p+1)}$ je diagonální matice se singulárními hodnotami $\sigma_1 \geq \sigma_2 \geq ... \geq \sigma_{p+1} \geq 0$ na diagonále.
...
&= \mathbf{V}(\boldsymbol{\Sigma}^T\boldsymbol{\Sigma})^{-1}\mathbf{V}^T\mathbf{V}\boldsymbol{\Sigma}^T\mathbf{U}^T\mathbf{Y} \quad \text{(využili jsme $\mathbf{V}^T = \mathbf{V}^{-1}$)} \\
...
\item Na obrázku 2: {\bf modré body} jsou trénovací data $(x_i, Y_i)$, {\bf červené body} jsou predikce $\hat{Y}_i$, {\bf modré křížky} jsou střední hodnoty $(x_i, \mathbb{E}(Y_i))$, {\bf modrá čerchovaná čára} je skutečná přímka $y = \mathbf{w}^T \mathbf{x}$ (neznámá), a {\bf červená čára} je naše odhadnutá přímka $\hat{y} = \hat{\mathbf{w}}^T \mathbf{x}$. Nejlepší odhad by byl, kdyby se červená přímka dostala na modrou čerchovanou čáru.
\end{itemize}
...
\begin{enumerate}
    \item Přigenerovat další data či odebrat existující.
...
Pro výpočet inverze matice $\mathbf{X}^T\mathbf{X}$ použijeme Gauss-Jordanovu eliminaci na rozšířené matici $(\mathbf{X}^T\mathbf{X} \mid \mathbf{I})$:
...
protože pro $\mathbf{v} = (v_0, 0, \dots, 0)^T \neq \theta$ platí $\mathbf{X}\mathbf{v} = (v_0, \dots, v_0)^T \neq \theta$.
...
\item Predikce: $\hat{Y} = \varphi(\mathbf{x})^T \hat{\mathbf{w}}$.
...
\begin{itemize}
\item \href{https://youtu.be/Q81RR3yKn30?si=iRJRqO4delNlPQrP}{Regularization Part 1: Ridge (L2) Regression (StatQuest with Josh Starmer)} \hfill \textbf{[20~min]}
...
\]
\item První člen odpovídá \textbf{neodstranitelné chybě}, která je dána náhodností v modelu. Tato chyba se nazývá Bayesovská (angl. \textbf{Bayes error}). Druhý člen se značí $\text{MSE}(\hat{Y})$ a nazývá \textbf{střední kvadratická chyba odhadu} $\hat{Y}$ parametru $\mathbb{E}Y$
...
kde bias $\hat{Y} = \mathbb{E}\hat{Y} - \mathbb{E}Y$ značí \textbf{vychýlení odhadu} (angl. \textbf{bias}).
...
\item \href{https://youtu.be/EuBBz3bI-aA?si=2hTRAyObYfLWdG2R}{Machine Learning Fundamentals: Bias and Variance (StatQuest with Josh Starmer)} \hfill \textbf{[6~min]}

\item \href{https://youtu.be/pvcEQfcO3pk?si=8oFJOnXPviM1zdAF}{Statistical Learning: 2.3 Model Selection and Bias Variance Tradeoff
 (Stanford Online)} \hfill \textbf{[10~min]}
...
\begin{itemize}
\item \href{https://youtu.be/yIYKR4sgzI8?si=p8gRQrpNXRz5oCfo}{StatQuest: Logistic Regression (StatQuest with Josh Starmer)} \hfill \textbf{[8~min]}
\end{itemize}
...
\item \textbf{Out-Of-Bag (OOB) statistika a limita:} Důsledkem metody Bootstrap (výběr s vracením) je, že část dat se do tréninku konkrétního stromu vůbec nedostane (tzv. OOB data).
\\
Pravděpodobnost, že konkrétní vzorek \textit{nebude} vybrán v jednom tahu, je $(1 - \frac{1}{N})$. Protože provádíme $N$ nezávislých tahů, pravděpodobnost, že nebude vybrán vůbec, je:
$$ P(\text{OOB}) = \left(1 - \frac{1}{N}\right)^N $$
Pro velmi velký dataset ($N \to \infty$) tato hodnota konverguje k převrácené hodnotě Eulerova čísla:
$$ \lim_{N \to \infty} \left(1 - \frac{1}{N}\right)^N = \frac{1}{e} \approx 0.368 $$
Z toho plyne, že každému stromu chybí zhruba \textbf{36,8\,\%} původních dat, která lze využít pro \textit{OOB Error} (validaci modelu zdarma bez nutnosti Cross-Validation).
...
\item \href{https://youtu.be/J4Wdy0Wc_xQ?si=Uk9bos0sQatMLDqt}{StatQuest: Random Forests - Building, Using and Evaluating (StatQuest with Josh Starmer)} \hfill \textbf{[9~min]}
\end{itemize}
...
\item \href{https://youtu.be/4jRBRDbJemM?si=KFIrkHlzfxtaVifB}{ROC and AUC, Clearly Explained!
 (StatQuest with Josh Starmer)} \hfill \textbf{[16~min]}

\item \href{https://youtu.be/Kdsp6soqA7o?si=9S04bcUbFDnriXJG}{Machine Learning Fundamentals: The Confusion Matrix (StatQuest with Josh Starmer)} \hfill \textbf{[7~min]}
...
\item \href{https://youtu.be/fSytzGwwBVw?si=-WlJx5vn9_aceiOm}{Machine Learning Fundamentals: Cross Validation (StatQuest with Josh Starmer)} \hfill \textbf{[8~min]}

\item \href{https://youtu.be/XXFLFWHP9Nc?si=7IRnlvnDji3LZZ0b}{11.5 Nested CV for Algorithm Selection (L11 Model Eval. Part 4) (Sebastian Raschka)} \hfill \textbf{[17~min]}
...
\item \href{https://youtu.be/NGf0voTMlcs?si=FmXSx7uRSdryZojC}{Regularization Part 2: Lasso (L1) Regression (StatQuest with Josh Starmer)} \hfill \textbf{[8~min]}

\item \href{https://youtu.be/1dKRdX9bfIo?si=nNSNtw0JFHOvPmoT}{Regularization Part 3: Elastic Net Regression (StatQuest with Josh Starmer)} \hfill \textbf{[5~min]}
...
\subsection{Zajímavá videa na YouTube}
\begin{itemize}
\item \href{https://youtu.be/4b5d3muPQmA?si=V0DAzHJelvt-UeR4}{
StatQuest: K-means clustering (StatQuest with Josh Starmer)} \hfill \textbf{[8~min]}
\end{itemize}
...
\begin{itemize}
\item \href{https://youtu.be/RDZUdRSDOok?si=Ds2vrsrBj9b9Drdc}{Clustering with DBSCAN, Clearly Explained!!! (StatQuest with Josh Starmer)} \hfill \textbf{[9~min]}
...
\subsection{Ukázka výpočtu asociačních pravidel}

\begin{itemize}

\item Mějme následující dataset z obchodu s potravinami, kde každý řádek reprezentuje jeden nákupní košík (transakci):

\begin{table}[h]
\centering
\begin{tabular}{c|ccccc}
\textbf{ID transakce} & \textbf{párky} & \textbf{hořčice} & \textbf{chléb} & \textbf{croissant} & \textbf{káva} \\
\hline
1 & 1 & 1 & 1 & 0 & 0 \\
2 & 1 & 0 & 1 & 0 & 1 \\
3 & 0 & 0 & 1 & 1 & 1 \\
...
10 & 1 & 0 & 1 & 0 & 1 \\
\end{tabular}
\caption{Dataset nákupních košíků (1 = položka zakoupena, 0 = položka nezakoupena).}
\label{tab:shopping_data}
\end{table}

\item {\bf Krok 1: Výpočet podpory jednotlivých položek}

Nejprve spočítáme, jak často se jednotlivé položky v datech vyskytují (viz tabulka \ref{tab:item_support}):

\begin{table}[h]
\centering
\begin{tabular}{c|c|c}
\textbf{Položka} & \textbf{Počet výskytů} & \textbf{Podpora} $T(\cdot)$ \\
\hline
párky & 6 & $6/10 = 0.6$ \\
hořčice & 6 & $6/10 = 0.6$ \\
chléb & 8 & $8/10 = 0.8$ \\
croissant & 4 & $4/10 = 0.4$ \\
káva & 6 & $6/10 = 0.6$ \\
\end{tabular}
\caption{Podpora jednotlivých položek.}
\label{tab:item_support}
\end{table}

\item {\bf Krok 2: Výpočet podpory dvojic položek}

Nyní spočítáme podporu vybraných dvojic položek (množin $\mathcal{K}$ o velikosti 2), jak ukazuje tabulka \ref{tab:pair_support}:

\begin{table}[h]
\centering
\small
\begin{tabular}{c|c|c}
\textbf{Dvojice položek} & \textbf{Počet výskytů} & \textbf{Podpora} $T(\cdot)$ \\
\hline
\{párky, hořčice\} & 4 & $4/10 = 0.4$ \\
\{párky, chléb\} & 5 & $5/10 = 0.5$ \\
\{hořčice, chléb\} & 4 & $4/10 = 0.4$ \\
\{croissant, káva\} & 3 & $3/10 = 0.3$ \\
\{chléb, káva\} & 4 & $4/10 = 0.4$ \\
\end{tabular}
\caption{Podpora vybraných dvojic položek.}
\label{tab:pair_support}
\end{table}

\item {\bf Krok 3: Analýza asociačního pravidla} $\{\texttt{párky}\} \Rightarrow \{\texttt{hořčice}\}$

Uvažujme pravidlo: "Pokud zákazník koupí párky, koupí i hořčici."

\begin{itemize}
\item {\bf Podpora pravidla:}
$$T(\{\texttt{párky}\} \Rightarrow \{\texttt{hořčice}\}) = T(\{\texttt{párky}, \texttt{hořčice}\}) = \frac{4}{10} = 0.4$$

\item {\bf Spolehlivost (confidence):}
$$C(\{\texttt{párky}\} \Rightarrow \{\texttt{hořčice}\}) = \frac{T(\{\texttt{párky}, \texttt{hořčice}\})}{T(\{\texttt{párky}\})} = \frac{0.4}{0.6} = 0.667$$

{\bf Interpretace:} Pokud zákazník koupí párky, v 66.7\% případů koupí i hořčici.

\item {\bf Zdvih (lift):}
$$L(\{\texttt{párky}\} \Rightarrow \{\texttt{hořčice}\}) = \frac{C(\{\texttt{párky}\} \Rightarrow \{\texttt{hořčice}\})}{T(\{\texttt{hořčice}\})} = \frac{0.667}{0.6} = 1.111$$

{\bf Interpretace:} Zdvih $> 1$ znamená pozitivní korelaci. Zákazníci, kteří kupují párky, kupují hořčici o 11.1\% častěji než průměrný zákazník.

\item {\bf Pokrytí (coverage):}
$$\text{Coverage}(\{\texttt{párky}\} \Rightarrow \{\texttt{hořčice}\}) = \frac{T(\{\texttt{párky}, \texttt{hořčice}\})}{T(\{\texttt{hořčice}\})} = \frac{0.4}{0.6} = 0.667$$

{\bf Interpretace:} Ve 66.7\% případů, kdy zákazník koupil hořčici, lze to vysvětlit tím, že koupil i párky.
\end{itemize}

\item {\bf Krok 4: Ověření proti prahům}

Předpokládejme, že jsme zvolili minimální prahy:
\begin{itemize}
    \item Minimální podpora: $t = 0.3$
    \item Minimální spolehlivost: $c = 0.6$
\end{itemize}

Pravidlo $\{\texttt{párky}\} \Rightarrow \{\texttt{hořčice}\}$ {\bf projde do finálního výstupu}, pokud splňuje obě podmínky:
$$T(\{\texttt{párky}\} \Rightarrow \{\texttt{hořčice}\}) > t \quad \text{a} \quad C(\{\texttt{párky}\} \Rightarrow \{\texttt{hořčice}\}) > c$$

Ověření:
\begin{align*}
T(\{\texttt{párky}\} \Rightarrow \{\texttt{hořčice}\}) &= 0.4 > 0.3 \quad \checkmark \\
C(\{\texttt{párky}\} \Rightarrow \{\texttt{hořčice}\}) &= 0.667 > 0.6 \quad \checkmark
\end{align*}

{\bf Závěr:} Pravidlo $\{\texttt{párky}\} \Rightarrow \{\texttt{hořčice}\}$ \textbf{je součástí finálního výstupu} asociační analýzy, protože splňuje obě podmínky.
...
\item \href{https://youtu.be/lG4VkPoG3ko?si=h7YEb7bAs9GE1l3W}{The medical test paradox, and redesigning Bayes' rule
 ...
 \item \href{https://youtu.be/cy8r7WSuT1I?si=l3B86oTKAPq2853W}{Why $\pi$ is in the normal distribution (beyond integral tricks) (3Blue1Brown)} \hfill\textbf{[24~min]}
  ...
  \item \href{https://youtu.be/IaSGqQa5O-M?si=SVnVv0O9h5_c7pB_}{Convolutions $\vert$ Why X+Y in probability is a beautiful mess
...
\item \href{https://youtu.be/wjZofJX0v4M?si=jsV5Va9NmxVQVb_m}{Transformers, the tech behind LLMs $\vert$ Deep Learning Chapter 5 (3Blue1Brown)} \hfill \textbf{[27~min]}

 \item \href{https://youtu.be/eMlx5fFNoYc?si=JrCksJ0LxjPA8Vub}{Attention in transformers, step-by-step $\vert$ Deep Learning Chapter 6 (3Blue1Brown)} \hfill \textbf{[26~min]}
...
Predikce: \hyperref[sec:bazove_funkce_model]{$\hat{Y} = \boldsymbol{\varphi}(\mathbf{x})^T \hat{\mathbf{w}}$}
\end{itemize}
